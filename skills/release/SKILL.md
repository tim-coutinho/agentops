...
`/release` MUST run `./scripts/ci-local-release.sh` first. If it fails, stop the release workflow and report the failing checks.
...
| Local CI parity gate | `./scripts/ci-local-release.sh` | **Block** — must pass before release |
...
If `--skip-checks` is passed, skip ad-hoc test/lint detection only. It does **not** skip `./scripts/ci-local-release.sh`.
